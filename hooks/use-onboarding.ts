...
export function useOnboarding(onDone: () => void) {
  ...
  const gauge = useMemo(() => Math.min(100, Math.round(((safety + growth) / 85) * 100)), [growth, safety]);
  ...
  async function finish() {
    ...
    await db.transaction("rw", db.allocationRules, db.settings, db.funds, async () => {
  ...
  return { step, setStep, preset, life, safety, growth, monthly, setMonthly, months, setMonths, stability, setStability, risk, setRisk, total, gauge, error, choosePreset, adjust, finish };
}
