...
const checks = [
  ...
  [packageSource.includes('"name": "poolamkoo"') && packageSource.includes('"version": "0.29.0"') && appVersionSource.includes('APP_VERSION = "0.29.0"'), "Package and runtime versions must use the canonical Poolamkoo v0.29.0 identity"],
  [rootLayout.includes("Poolamkoo open-source contributors") && serviceWorker.includes('const CACHE = "poolamkoo-v53"') && serviceWorker.includes('/logo-poolamkoo.svg'), "Active runtime branding and PWA cache/assets must use the Poolamkoo spelling"],
  ...
  [serviceWorker.includes('const CACHE = "poolamkoo-v53"') && serviceWorker.includes('event.data?.type === "SKIP_WAITING"') && serviceWorker.indexOf('addEventListener("message"') < serviceWorker.indexOf("self.skipWaiting()"), "Service worker updates must not skip waiting unconditionally during install"],
  ...
  [portfolioSource.includes("positivePrice(quote?.priceToman)") && portfolioSource.includes("marketPrice ?? manualPrice") && marketHook.includes("targetDescriptors") && marketHook.includes("params.append(target.source, target.id)") && marketHook.includes('quote.source !== "tsetmc"') && marketHook.includes('quote.source !== "tindex"'), "Linked exchange assets must request only source-aware market IDs and retain safe manual pricing fallback"],
...
console.log("Regression checks passed: direct TSETMC exchange data, privacy-safe report export, draggable mobile navigation, focused search, safe workspace motion, release browser gate, workspace-only PWA entry, safe local data, privacy boundaries, market cache, and responsive UI are wired.");
