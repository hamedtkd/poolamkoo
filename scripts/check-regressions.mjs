...
const checks = [
  ...
  [packageSource.includes('"name": "poolamkoo"') && packageSource.includes('"version": "0.34.0"') && appVersionSource.includes('APP_VERSION = "0.34.0"'), "Package and runtime versions must use the canonical Poolamkoo v0.34.0 identity"],
  [rootLayout.includes("Poolamkoo open-source contributors") && serviceWorker.includes('const CACHE = "poolamkoo-v58"') && serviceWorker.includes('/logo-poolamkoo.svg'), "Active runtime branding and PWA cache/assets must use the Poolamkoo spelling"],
  ...
  [serviceWorker.includes('const CACHE = "poolamkoo-v58"') && serviceWorker.includes('event.data?.type === "SKIP_WAITING"') && serviceWorker.indexOf('addEventListener("message"') < serviceWorker.indexOf("self.skipWaiting()"), "Service worker updates must not skip waiting unconditionally during install"],
