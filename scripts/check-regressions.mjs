...
const loanCalculationsSource = await read("lib/loans/calculations.ts");
const loanPortableSource = await read("lib/loans/portable.ts");
const loanStoreSource = await read("lib/loans/store.ts");
const loanPaymentsSource = await read("lib/loans/payments.ts");
...
const checks = [
  [packageSource.includes('"check:release": "npm run check && npm run build && npm run test:browser:release:built"') && releaseSmokeSource.includes("schema 6 profile must upgrade in place through schema 9") && releaseSmokeSource.includes("Tindex and TSETMC rows with the same raw marketId must coexist after migration") && releaseSmokeSource.includes("schema 8 migration must create an opening fund-ledger row before schema 9 loan stores are added") && schema6FixtureSource.includes("LEGACY_SCHEMA6_NATIVE_VERSION = 60") && schema6FixtureSource.includes("CURRENT_SCHEMA9_NATIVE_VERSION = 90") && releaseSmokeSource.includes("landing-to-workspace navigation") && releaseSmokeSource.includes("fresh onboarding") && releaseSmokeSource.includes('clientNavigate(client, "/settings"') && releaseSmokeSource.includes("workspace content must remain visible after client navigation") && releaseSmokeSource.includes("shared dialog content must never render blank under normal motion preference") && releaseSmokeSource.includes("dashboard critical content must be visible with normal motion preference") && releaseSmokeSource.includes("workspace must compile tailwindcss-animated stagger utilities with distinct delays") && releaseSmokeSource.includes("workspace service worker registration") && releaseSmokeSource.includes("reports must expose privacy-safe export controls") && releaseSmokeSource.includes("pending investment purchases must group repeated asset cards by income") && releaseSmokeSource.includes("drag-to-dismiss mobile drawer") && releaseSmokeSource.includes("workspace service worker must not cache the public landing navigation") && releaseSmokeWorkflowSource.includes("npm run check:release"), "Release browser gate must verify schema 6→9 migration, including fund-ledger continuity and loan stores, plus public/workspace navigation, onboarding/bootstrap, client route continuity, reports and PWA runtime boundaries"],

  [landingPageSource.includes("LandingHero") && landingPageSource.includes("LandingSections") && cinematicHeroSource.includes("شروع رایگان") && cinematicHeroSource.includes("داده مالی روی دستگاه شما") && cinematicHeroSource.includes("پول میرسه"), "Public root must present a clear Persian landing page before entering the financial app"],
  ...
  [packageSource.includes('"name": "poolamkoo"') && packageSource.includes('"version": "1.3.0"') && appVersionSource.includes('APP_VERSION = "1.3.0"'), "Package and runtime versions must use the canonical Poolamkoo v1.3.0 identity"],
  [rootLayout.includes("Poolamkoo open-source contributors") && serviceWorker.includes('const CACHE = "poolamkoo-v72"') && serviceWorker.includes('/logo-poolamkoo.svg'), "Active runtime branding and PWA cache/assets must use the Poolamkoo spelling"],
  ...
  [serviceWorker.includes('const CACHE = "poolamkoo-v72"') && serviceWorker.includes('event.data?.type === "SKIP_WAITING"') && serviceWorker.indexOf('addEventListener("message"') < serviceWorker.indexOf("self.skipWaiting()"), "Service worker updates must not skip waiting unconditionally during install"],
  ...
  [db.includes('this.version(6).stores(storesV6)') && db.includes('this.version(7).stores(storesV7)') && db.includes('this.version(8).stores(storesV8)') && db.includes('this.version(LOCAL_DATABASE_SCHEMA_VERSION).stores(storesV9)') && appVersionSource.includes('LOCAL_DATABASE_SCHEMA_VERSION = 9') && dbSchemaSource.includes('fundMovements: "++id, fundId, type, source, happenedAt, createdAt"') && dbSchemaSource.includes('loanPayments: "++id, loanId, [loanId+installmentNo], dueAt, paidAt"') && db.includes('legacyFundOpeningMovement') && db.includes('recoverySnapshots') && db.includes('appMeta'), "Data safety schema must preserve the v6-v8 migration chain and add schema 9 loan stores in place"],
  ...
  [dataPortabilitySource.includes("assertPortableFundLedger") && dataPortabilitySource.includes("assertPortableLoanData") && db.includes("normalizePortableFundLedger") && db.includes("normalizePortableLoanData") && loanPortableSource.includes("normalizePortableLoanData"), "Backup restore must preserve the schema 8 fund ledger and normalize schema 9 loan tables while remaining backward-compatible"],
  [loanCalculationsSource.includes("loanBreakEvenAnnualRate") && loanCalculationsSource.includes("loanScenarioEndingBalance") && loanStoreSource.includes("createLoan") && loanStoreSource.includes("deleteLoan") && loanPaymentsSource.includes("recordLoanPayment") && loanPaymentsSource.includes("loanPaymentContributionTotals"), "Loan core must keep pure financial math separate from local CRUD and the payment ledger"],
  ...
  [marketValuationSource.includes("quote.marketSource ??") && marketValuationSource.includes("exchangeSource === target.source") && marketValuationSource.includes("quote.marketId === target.marketId") && portfolioSource.includes("resolveAssetValuation") && dashboardMetricsSource.includes("resolveAssetValuation") && reportsDataSource.includes("resolveAssetValuation") && marketWatchHelperSource.includes("marketQuoteForTarget") && marketAlertHookSource.includes("marketQuoteForTarget"), "Linked market identity must stay independent from the provider that returned the live quote"],
  ...
  [marketSearchSource.includes("new TsetmcProvider") && marketSearchSource.includes("new BrsApiProvider") && marketSearchSource.includes("new TindexProvider") && !marketSearchSource.includes("status: 502"), "Exchange search must stay multi-source and return a usable 200 response instead of a dependency 502"],
  ...
  [marketApi.includes("parseExchangeTargets") && marketApi.includes("new TsetmcProvider().getTargetQuotes(targets)") && marketApi.includes("brs.getExchangeQuotes") && marketApi.includes("tindex.getTargetQuotes") && types.includes("marketSource?: ExchangeMarketSource"), "Market API must accept structured exchange targets and preserve logical identity across all live providers"],
  ...
  [marketApi.indexOf("new TsetmcProvider().getTargetQuotes(targets)") < marketApi.indexOf("brs.getExchangeQuotes") && marketApi.indexOf("brs.getExchangeQuotes") < marketApi.indexOf("tindex.getTargetQuotes") && marketApi.includes("missingTargets") && marketApi.includes("summarizeMarketHealth"), "Exchange quote resolution must try TSETMC, then BrsApi, then Tindex while preserving partial results"],
  [marketApi.includes("health,") && marketSearchSource.includes("summarizeMarketHealth") && marketHistoryApiSource.includes("summarizeMarketHealth") && marketHook.includes("MarketHealthSummary") && marketHook.includes("setHealth(data.health)") && marketRefreshButton.includes("controls.health?.degraded"), "Quote/search/history health metadata must reach the client and make degraded refresh state visible"],
  ...
  [portfolioSource.includes("resolveAssetValuation") && marketValuationSource.includes("positivePrice(quote?.priceToman)") && marketValuationSource.includes("manualPriceToman") && marketHook.includes("targetDescriptors") && marketHook.includes('params.append("target", JSON.stringify(target))') && marketHook.includes("quote.marketSource ??"), "Linked exchange assets must send source/id/symbol targets and accept another provider without losing the original identity"],
  [marketApi.includes("needsCoreFallback") && marketApi.includes("getCoreQuotes") && marketPrioritySource.includes("for (const quote of primary)"), "Core-rate coverage must preserve complete primary data and request only genuinely missing supported symbols"],
  [tindexProviderSource.includes("/boards") && tindexProviderSource.includes("USD-EXCHANGE-RATE") && tindexProviderSource.includes("GOLD-18K") && tindexProviderSource.includes("btc") && tindexProviderSource.includes("MARKET_CACHE_SECONDS.tindexCoreQuotes") && marketQuotaSource.includes("tindexCoreQuotes: 1800") && !tindexProviderSource.includes('cache: "no-store"'), "Tindex core-rate calls must stay consolidated and protected by shared caching"],
  [tindexProviderSource.includes("slice(0, 1)") && tindexProviderSource.includes("stock-market/symbol") && tindexProviderSource.includes("MARKET_CACHE_SECONDS.tindexExchangeQuote") && marketQuotaSource.includes("tindexExchangeQuote: 3600") && tindexProviderSource.includes("requestDomainHeaders") && tindexProviderSource.includes("Origin") && tindexProviderSource.includes("Referer"), "Tindex exchange calls must refresh conservatively and identify the configured project domain"],
  [marketSourceLabelSource.includes("منبع داده: TSETMC") && marketSourceLabelSource.includes("https://www.tsetmc.com") && marketSourceLabelSource.includes("منبع داده: Tindex") && exchangePickerSource.includes("MarketSourceLabel"), "Exchange quotes must visibly attribute the actual provider without exposing internal ordering"],
  [landingSectionsSource.includes("TSETMC، BrsApi و Tindex") && landingSectionsSource.includes("اسپانسر پروژه پولم‌کو") && landingSectionsSource.includes("COMMUNITY_LINKS.tindex") && publicShellSource.includes("منبع داده و اسپانسر پروژه") && envExampleSource.includes("Origin/Referer headers"), "Public pages must credit Tindex as a data source and sponsor with a direct homepage link and domain-bound server configuration"],
  [marketHistoryApiSource.includes("new TsetmcProvider") && marketHistoryApiSource.includes("new BrsApiProvider") && marketHistoryApiSource.includes("new TindexProvider") && marketHistoryApiSource.includes("getExchangeCandles") && marketHistoryApiSource.includes("getIndicatorCandles") && marketHistoryApiSource.includes("status: 200") && marketHistoryApiSource.includes("s-maxage=3600"), "Market history must remain multi-source, cached and graceful when one provider is unavailable"],
  ...
  [tsetmcProviderSource.includes("parseTsetmcHistoryPayload") && marketProvider.includes("parseBrsTsetmcHistoryPayload") && tindexProviderSource.includes("parseTindexCandlesPayload"), "Exchange history parsers must normalize real OHLC data from TSETMC, BrsApi and Tindex"],
