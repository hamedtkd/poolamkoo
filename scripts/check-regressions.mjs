...
const checks = [
  ...
  [packageSource.includes('"name": "poolamkoo"') && packageSource.includes('"version": "1.0.0-rc.2"') && appVersionSource.includes('APP_VERSION = "1.0.0-rc.2"'), "Package and runtime versions must use the canonical Poolamkoo v1.0.0-rc.2 identity"],
  ...
  [productTour.includes('data-tour-shade="top"') && productTour.includes('data-tour-shade="right"') && productTour.includes('data-tour-spotlight="true"') && productTour.includes('در حال نمایش: {tour.step.location}') && !productTour.includes('9999px') && appTopbar.includes('data-tour="global-search"') && tourHook.includes('title: "جست‌وجو همیشه در دسترس است"') && !tourHook.includes("{ target: '[data-tour=\"reports\"]'"), "Product tour must keep the target outside the overlay and use visible responsive anchors"],
