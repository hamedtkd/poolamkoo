...
const checks = [
  ...
  [packageSource.includes('"name": "poolamkoo"') && packageSource.includes('"version": "0.32.0"') && appVersionSource.includes('APP_VERSION = "0.32.0"'), "Package and runtime versions must use the canonical Poolamkoo v0.32.0 identity"],
  [rootLayout.includes("Poolamkoo open-source contributors") && serviceWorker.includes('const CACHE = "poolamkoo-v56"') && serviceWorker.includes('/logo-poolamkoo.svg'), "Active runtime branding and PWA cache/assets must use the Poolamkoo spelling"],
  ...
  [serviceWorker.includes('const CACHE = "poolamkoo-v56"') && serviceWorker.includes('event.data?.type === "SKIP_WAITING"') && serviceWorker.indexOf('addEventListener("message"') < serviceWorker.indexOf("self.skipWaiting()"), "Service worker updates must not skip waiting unconditionally during install"],
...
console.log("Regression checks passed: direct TSETMC exchange data, privacy-safe report export, draggable mobile navigation, focused search, safe workspace motion, release browser gate, workspace-only PWA entry, safe local data, privacy boundaries, partial market Snapshot continuity, market cache, and responsive UI are wired.");
