...
const checks = [
  [packageSource.includes('"check:release": "npm run check && npm run build && npm run test:browser:release:built"') && releaseSmokeSource.includes("landing-to-workspace navigation") && releaseSmokeSource.includes("fresh onboarding") && releaseSmokeSource.includes("جمع‌بندی تصمیمی این بازه") && releaseSmokeSource.includes("workspace service worker registration") && releaseSmokeWorkflowSource.includes("npm run check:release"), "Release browser gate must cover public/workspace navigation, onboarding/bootstrap, reports and PWA runtime boundaries"],
  ...
  [packageSource.includes('"name": "poolamkoo"') && packageSource.includes('"version": "0.26.1"') && appVersionSource.includes('APP_VERSION = "0.26.1"'), "Package and runtime versions must use the canonical Poolamkoo v0.26.1 identity"],
  [rootLayout.includes("Poolamkoo open-source contributors") && serviceWorker.includes('const CACHE = "poolamkoo-v44"') && serviceWorker.includes('/logo-poolamkoo.svg'), "Active runtime branding and PWA cache/assets must use the Poolamkoo spelling"],
  ...
  [serviceWorker.includes('const CACHE = "poolamkoo-v44"') && serviceWorker.includes('event.data?.type === "SKIP_WAITING"') && serviceWorker.indexOf('addEventListener("message"') < serviceWorker.indexOf("self.skipWaiting()"), "Service worker updates must not skip waiting unconditionally during install"],
...
console.log("Regression checks passed: release browser gate, theme-aware landing media, decision-focused reports, workspace-only PWA entry, safe local data, privacy boundaries, market cache, and responsive UI are wired.");
