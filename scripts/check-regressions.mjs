...
const checks = [
  [packageSource.includes('"check:release": "npm run check && npm run build && npm run test:browser:release:built"') && releaseSmokeSource.includes("schema 6 profile must upgrade in place to schema 7") && releaseSmokeSource.includes("Tindex and TSETMC rows with the same raw marketId must coexist after migration") && schema6FixtureSource.includes("LEGACY_SCHEMA6_NATIVE_VERSION = 60") && schema6FixtureSource.includes("CURRENT_SCHEMA7_NATIVE_VERSION = 70") && releaseSmokeSource.includes("landing-to-workspace navigation") && releaseSmokeSource.includes("fresh onboarding") && releaseSmokeSource.includes('clientNavigate(client, "/settings"') && releaseSmokeSource.includes("workspace content must remain visible after client navigation") && releaseSmokeSource.includes("shared dialog content must never render blank under normal motion preference") && releaseSmokeSource.includes("dashboard critical content must be visible with normal motion preference") && releaseSmokeSource.includes("workspace must compile tailwindcss-animated stagger utilities with distinct delays") && releaseSmokeSource.includes("workspace service worker registration") && releaseSmokeSource.includes("reports must expose privacy-safe export controls") && releaseSmokeSource.includes("drag-to-dismiss mobile drawer") && releaseSmokeWorkflowSource.includes("npm run check:release"), "Release browser gate must verify schema 6→7 migration plus public/workspace navigation, onboarding/bootstrap, client route continuity, reports and PWA runtime boundaries"],
  ...
  [packageSource.includes('"name": "poolamkoo"') && packageSource.includes('"version": "0.37.0"') && appVersionSource.includes('APP_VERSION = "0.37.0"'), "Package and runtime versions must use the canonical Poolamkoo v0.37.0 identity"],
  [rootLayout.includes("Poolamkoo open-source contributors") && serviceWorker.includes('const CACHE = "poolamkoo-v61"') && serviceWorker.includes('/logo-poolamkoo.svg'), "Active runtime branding and PWA cache/assets must use the Poolamkoo spelling"],
  ...
  [serviceWorker.includes('const CACHE = "poolamkoo-v61"') && serviceWorker.includes('event.data?.type === "SKIP_WAITING"') && serviceWorker.indexOf('addEventListener("message"') < serviceWorker.indexOf("self.skipWaiting()"), "Service worker updates must not skip waiting unconditionally during install"],
  ...
  [db.includes('this.version(6).stores(storesV6)') && db.includes('this.version(LOCAL_DATABASE_SCHEMA_VERSION).stores(storesV7)') && appVersionSource.includes('LOCAL_DATABASE_SCHEMA_VERSION = 7') && db.includes('recoverySnapshots') && db.includes('appMeta'), "Data safety schema must preserve v6 history while upgrading persisted market identity on schema 7"],
...
console.log("Regression checks passed: safe asset archive lifecycle, investment transaction correction integrity, automated schema 6→7 browser migration, persisted provider-scoped market identity, decision-safe Snapshot fallback, privacy-safe report export, draggable mobile navigation, focused search, warning-free release lint, safe workspace motion, release browser gate, workspace-only PWA entry, safe local data, privacy boundaries, market cache, and responsive UI are wired.");
