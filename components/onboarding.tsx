...
export function Onboarding({ onDone }: { onDone: () => void }) {
  ...
  return <main className="min-h-svh p-3 sm:p-6"><div className="mx-auto flex min-h-[calc(100svh-1.5rem)] max-w-5xl flex-col rounded-[32px] border bg-background/88 shadow-2xl backdrop-blur-xl sm:min-h-[calc(100svh-3rem)]">
    <header className="flex items-center justify-between border-b px-5 py-4 sm:px-8"><div className="flex items-center gap-3"><BrandLogo className="size-12" /><div><div className="type-caption text-muted-foreground">راه‌اندازی قانون پول شخصی</div><div className="type-label">پولم‌کو</div></div></div><div className="type-caption text-muted-foreground">مرحله {new Intl.NumberFormat("fa-IR").format(state.step + 1)} از ۴</div></header>
