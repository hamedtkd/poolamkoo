...
export function ReportsSection({ settings, rule, incomes, allocations, funds, fundMovements, allFundMovements, assets, transactions, periodTransactions, loans, loanPayments, periodLoanPayments, quotes, planItems, range }: {
  ...
  const columns: ColumnDef<DataTableFeatures, PerformanceRow, unknown>[] = [
    ...
    { accessorKey: "actual", header: () => <HelpLabel label="سهم فعلی" help={HELP.actual} />, cell: ({ row }) => row.original.valuationAvailable ? <SensitiveValue>{formatPercent(row.original.actual, 1)}</SensitiveValue> : <span className="text-muted-foreground">نامشخص</span> },
    { id: "drift", header: () => <HelpLabel label="فاصله از هدف" help={HELP.drift} />, cell: ({ row }) => row.original.valuationAvailable ? <DriftValue value={row.original.actual - row.original.target} /> : <span className="text-muted-foreground">نامشخص</span> },
...
function PerformanceValue({ row, settings }: { row: PerformanceRow; settings: AppSettings }) { if (!row.valuationAvailable) return <span className="text-muted-foreground">قیمت فعلی نداریم</span>; return <div><SensitiveValue>{formatMoney(row.value, settings.displayUnit)}</SensitiveValue><div className={cn("mt-1 text-[10px]", row.pricingReliable ? "text-muted-foreground" : "text-amber-700 dark:text-amber-300")}>{valuationPriceSourceLabel(row.priceSource)}</div></div>; }
function ReturnValue({ row, settings }: { row: PerformanceRow; settings: AppSettings }) { if (!row.valuationAvailable) return <span className="text-muted-foreground">قیمت فعلی نداریم</span>; return <div className={cn("type-strong", row.pnlPct >= 0 ? "text-profit" : "text-loss")}><SensitiveValue>{formatSignedPercent(row.pnlPct)}</SensitiveValue><div className="type-caption text-[10px]"><SensitiveValue>{formatSignedMoney(row.pnl, settings.displayUnit, true)}</SensitiveValue></div></div>; }
...
function PerformanceMobile({ row, settings }: { row: PerformanceRow; settings: AppSettings }) { const drift = row.actual - row.target; return <div className="p-4"><div className="flex items-start justify-between"><div><div className="type-strong">{row.name}</div><div className="mt-1 type-caption text-muted-foreground">{row.valuationAvailable ? <>ارزش <SensitiveValue>{formatMoney(row.value, settings.displayUnit)}</SensitiveValue> · {valuationPriceSourceLabel(row.priceSource)}</> : "قیمت فعلی نداریم"}</div></div>{row.valuationAvailable ? <SensitiveValue className={cn("type-strong", row.pnlPct >= 0 ? "text-profit" : "text-loss")}>{formatSignedPercent(row.pnlPct)}</SensitiveValue> : <span className="text-muted-foreground">—</span>}</div><div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs"><Small label="هدف" value={formatPercent(row.target, 0)} /><Small label="سهم فعلی" value={row.valuationAvailable ? formatPercent(row.actual, 1) : "نامشخص"} /><Small label="فاصله" value={row.valuationAvailable ? formatSignedPercent(drift) : "نامشخص"} /></div><div className="mt-2 type-caption text-muted-foreground">بازده از قیمت خرید واقعی شماست، نه تغییر روزانه بازار.</div></div>; }
