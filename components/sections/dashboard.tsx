...
export function DashboardSection({ settings, rule, incomes, funds, assets, transactions, quotes, snapshots, marketMode, marketLoading, marketLastUpdated, marketWarning, planItems, onRefreshMarket, onNewMoney, onOpenInvestments, onOpenFunds }: {
  ...
  return <div className="space-y-4 sm:space-y-5">
    ...
    <div className="grid gap-4 xl:grid-cols-[1.05fr_1.95fr]">
      <Card className="overflow-hidden"><CardHeader className="flex flex-row items-center justify-between"><div><CardTitle>قانون پول فعلی</CardTitle><p className="mt-1 type-caption text-muted-foreground">{rule?.name ?? "بدون قانون فعال"}</p></div><Badge>{rule?.preset === "custom" ? "شخصی" : "پیشنهادی"}</Badge></CardHeader><CardContent><div className="grid gap-4 sm:grid-cols-[210px_1fr] sm:items-center"><div className="mx-auto"><ArcGauge value={Math.min(100, decisionGauge)} label="تمرکز روی امنیت + رشد" size={190} stroke={23}/></div><div className="space-y-3">{rule && <><AllocationRow label="زندگی" value={rule.lifePct}/><AllocationRow label="امنیت" value={rule.safetyPct}/><AllocationRow label="رشد" value={rule.growthPct}/></>}</div></div></CardContent></Card>
...
function SummaryCard({ icon, label, value, sub, positive }: { icon: React.ReactNode; label: string; value: string; sub: string; positive?: boolean }) { return <Card><CardContent className="p-4 sm:p-5"><div className="flex items-start justify-between"><div><div className="type-caption text-muted-foreground">{label}</div><SensitiveValue className="mt-2 type-section-title tabular-nums sm:text-2xl">{value}</SensitiveValue></div><div className="grid size-9 place-items-center rounded-xl bg-primary/10 text-primary [&_svg]:size-5">{icon}</div></div><div className={cn("mt-3 text-[11px]", positive === false ? "text-destructive" : "text-muted-foreground")}><SensitiveValue>{sub}</SensitiveValue></div></CardContent></Card>; }
