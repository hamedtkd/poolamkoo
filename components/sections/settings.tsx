...
export function SettingsSection({ settings, rule }: { settings: AppSettings; rule?: AllocationRule }) {
  ...
  return (
    <div className="space-y-5">
      <header><div className="text-xs font-semibold text-primary">تنظیمات</div><h1 className="mt-1 text-2xl font-black sm:text-3xl">پولم‌کو را برای خودت تنظیم کن</h1><p className="mt-1 text-sm text-muted-foreground">واحد پول، ظاهر، قانون تخصیص، ذخیره اضطراری و بکاپ همگی قابل تغییرند.</p></header>
      {manager.message && <div className="rounded-2xl border border-primary/20 bg-primary/7 px-4 py-3 text-sm font-semibold text-primary">{manager.message}</div>}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><RiBrush2Line className="text-primary" /> ظاهر و واحد پول</CardTitle></CardHeader><CardContent className="space-y-5">
          <SettingField label="واحد نمایشی"><Select value={settings.displayUnit} onValueChange={(value) => void manager.updateSettings({ displayUnit: value as AppSettings["displayUnit"] })} options={[{ value: "toman", label: "تومان" }, { value: "rial", label: "ریال" }]} /><p className="text-xs text-muted-foreground">داده‌ها canonical در تومان ذخیره می‌شوند؛ تغییر واحد فقط نمایش را عوض می‌کند.</p></SettingField>
        ...
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><RiWallet3Line className="text-primary" /> قانون پول من</CardTitle></CardHeader><CardContent>
          <form onSubmit={manager.saveRule} className="space-y-5">
            ...
            <div className={cn("flex items-center justify-between rounded-xl px-3 py-2 text-sm font-bold", Math.round(total) === 100 ? "bg-primary/8 text-primary" : "bg-destructive/10 text-destructive")}><span>مجموع</span><span>{formatPercent(total, 0)}</span></div>
        ...
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><RiShieldKeyholeLine className="text-primary" /> امنیت مالی و سبک درآمد</CardTitle></CardHeader><CardContent className="space-y-4">
          ...
          <form onSubmit={manager.saveEmergencyPlan} className="space-y-4">
            ...
            <div className="rounded-2xl bg-muted/60 p-3 text-sm"><div className="text-xs text-muted-foreground">هدف تقریبی صندوق اضطراری</div><div className="mt-1 font-black">{formatMoney(manager.emergency.watch("monthlyEssentialToman") * manager.emergency.watch("emergencyMonths"), settings.displayUnit)}</div></div>
        ...
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><RiArchiveLine className="text-primary" /> بکاپ و بازیابی</CardTitle></CardHeader><CardContent className="space-y-4">
          <div className="flex items-center justify-between rounded-2xl border p-3"><div><div className="font-bold">رمزنگاری بکاپ</div><div className="text-xs text-muted-foreground">AES-GCM با کلید ساخته‌شده از رمز شما</div></div><Switch checked={manager.encryptBackup} onCheckedChange={manager.setEncryptBackup} /></div>
      ...
      <Card><CardContent className="grid gap-4 p-5 sm:grid-cols-[1fr_auto] sm:items-center"><div><div className="flex items-center gap-2 font-black"><RiInstallLine className="text-primary" /> نصب PWA</div><p className="mt-1 text-sm text-muted-foreground">پولم‌کو را روی Home Screen نصب کن تا مثل اپ مستقل اجرا شود.</p></div><Button variant="outline" onClick={() => void manager.installPwa()}>نصب روی دستگاه</Button></CardContent></Card>
    </div>
  );
}
...
function AllocationSlider({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <div className="space-y-2"><div className="flex items-center justify-between"><Label>{label}</Label><span className="text-sm font-black text-primary">{formatPercent(value, 0)}</span></div><Slider min={0} max={100} step={5} value={[value]} onValueChange={([next]) => onChange(next)} /></div>; }
