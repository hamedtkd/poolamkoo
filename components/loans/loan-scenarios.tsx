...
import { formatMoney, formatNumber, formatPercent, formatSignedMoney } from "@/lib/format";
...
export function LoanScenarios({ view, settings }: { view: LoanView; settings: AppSettings }) {
  ...
  return <div className="space-y-4">
    ...
    <Card><CardHeader><CardTitle>شبیه‌سازی شوک بازار</CardTitle><p className="mt-1 type-caption leading-6 text-muted-foreground">یک شوک یکسان فقط روی ارزش دارایی‌های متصل اعمال می‌شود؛ ذخیره اقساط و وجه نقد ثابت می‌مانند. این آزمون پیش‌بینی بازار نیست.</p></CardHeader><CardContent>{hasAvailableAsset ? <><div className="flex flex-wrap gap-2">{[-50, -30, -20, -10].map((value) => <Button key={value} variant={shockPct === value ? "default" : "outline"} onClick={() => setShockPct(value)}>افت {formatNumber(Math.abs(value))}٪</Button>)}</div><div className="mt-4 rounded-2xl border bg-muted/20 p-4"><div className="type-strong">سناریو: افت {formatNumber(Math.abs(shockPct))}٪</div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><ScenarioStat label="ارزش دارایی پس از شوک" value={formatMoney(stressedAssetValueToman, settings.displayUnit, true)}/><ScenarioStat label="اثر روی سرمایه" value={formatSignedMoney(shockDeltaToman, settings.displayUnit, true)} tone={shockDeltaToman >= 0 ? "profit" : "loss"}/><ScenarioStat label="معادل" value={`${formatNumber(shockInstallments, 1)} قسط`}/><ScenarioStat label="وضعیت" value={manageability}/></div></div><p className="mt-3 type-caption text-muted-foreground">اثر خالص پس از شوک: {formatSignedMoney(stressedNetEffectToman, settings.displayUnit, true)}</p></> : <div className="type-caption text-muted-foreground">برای اجرای شبیه‌سازی بازار، ابتدا یک دارایی متصل کنید.</div>}</CardContent></Card>
  </div>;
}
