    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            از مبلغ آزاد همین پول، یک هدف جدید بساز. مجموع برنامه هیچ‌وقت از مبلغ ورودی بیشتر نمی‌شود.
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={state.submit}>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Controller name="bucket" control={form.control} render={({ field }) => (
              <div className="space-y-2"><Label>بخش</Label><Select value={field.value} onValueChange={field.onChange} options={bucketOptions} /></div>
            )} />
            <Controller name="targetType" control={form.control} render={({ field }) => (
              <div className="space-y-2"><Label>نوع کارت</Label><Select value={field.value} onValueChange={(value) => { field.onChange(value); form.setValue("targetId", null); }} options={state.targetTypeOptions} /></div>
            )} />
          </div>
          {targetType !== "bucket" && (
