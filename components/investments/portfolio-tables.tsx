...
function PriceCell({ row, settings }: { row: PositionRow; settings: AppSettings }) {
  return <div><SensitiveValue>{formatMoney(row.price, settings.displayUnit, true)}</SensitiveValue>{row.quote && <div className="mt-1 flex items-center gap-1"><Badge className="text-primary">بازار</Badge><MarketSourceLabel source={row.quote.source} compact className="text-[9px] text-muted-foreground" /></div>}</div>;
}
