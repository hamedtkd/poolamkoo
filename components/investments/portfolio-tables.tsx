...
export function PortfolioTables({ positions, allocationRows, transactions, assets, settings, onTransaction, onEditAsset, onArchiveAsset, onRestoreAsset, onEditTransaction, onDeleteTransaction }: {
  ...
  const positionColumns: ColumnDef<DataTableFeatures, PositionRow, unknown>[] = [
    ...
    { accessorKey: "returnPct", header: "بازده از خرید", cell: ({ row }) => <ReturnCell row={row.original} settings={settings} /> },
...
function ReturnCell({ row, settings }: { row: PositionRow; settings: AppSettings }) {
  if (!row.valuationAvailable) return <span className="type-caption text-muted-foreground">قیمت فعلی نداریم</span>;
  return <div className={cn("type-strong", row.returnPct >= 0 ? "text-profit" : "text-loss")}><SensitiveValue>{formatSignedPercent(row.returnPct)}</SensitiveValue><div className="type-caption text-[10px]"><SensitiveValue>{formatSignedMoney(row.unrealized, settings.displayUnit, true)}</SensitiveValue></div></div>;
}
