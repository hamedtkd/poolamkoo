...
export function PortfolioTables({ positions, allocationRows, transactions, assets, settings, onTransaction, onEditAsset, onArchiveAsset, onDeleteTransaction }: {
  ...
  const transactionColumns: ColumnDef<DataTableFeatures, InvestmentTransaction, unknown>[] = [
    ...
    { id: "actions", header: "", cell: ({ row }) => <Button size="icon" variant="ghost" className="size-8 text-destructive" aria-label="حذف تراکنش" onClick={() => row.original.id && onDeleteTransaction(row.original.id)}><RiDeleteBin6Line /></Button> },
  ];

  return <><Card><CardHeader><CardTitle>دارایی‌های سبد</CardTitle></CardHeader><CardContent><DataTable data={positions} columns={positionColumns} searchPlaceholder="جست‌وجوی دارایی..." mobileCard={(row) => <AssetMobileCard row={row} allocation={allocationMap.get(row.asset.id)} settings={settings} onTx={() => onTransaction(row.asset)} onEdit={() => onEditAsset(row.asset)} onArchive={() => onArchiveAsset(row.asset)} />} /></CardContent></Card><Card><CardHeader><CardTitle>تراکنش‌های سرمایه‌گذاری</CardTitle></CardHeader><CardContent><DataTable data={[...transactions].sort((a, b) => b.happenedAt.localeCompare(a.happenedAt))} columns={transactionColumns} searchPlaceholder="جست‌وجوی تراکنش..." mobileCard={(row) => <TransactionMobileCard row={row} assets={assets} settings={settings} onDelete={() => row.id && onDeleteTransaction(row.id)} />} /></CardContent></Card></>;
}

function PriceCell({ row, settings }: { row: PositionRow; settings: AppSettings }) {
  return <div><SensitiveValue>{formatMoney(row.price, settings.displayUnit, true)}</SensitiveValue>{row.priceSource === "market" && row.quote ? <div className="mt-1 flex items-center gap-1"><Badge className="text-primary">بازار</Badge><MarketSourceLabel source={row.quote.source} compact className="text-[9px] text-muted-foreground" /></div> : row.priceSource === "cost-basis" && row.qty > 0 ? <div className="mt-1 text-[9px] text-muted-foreground">fallback بهای خرید</div> : null}</div>;
}
