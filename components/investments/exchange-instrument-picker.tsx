...
import type { AppSettings, MarketInstrument } from "@/lib/types";
...
export function ExchangeInstrumentPicker({ selected, settings, onSelect, onClear }: {
  ...
  function search(value: string) {
    ...
    timerRef.current = window.setTimeout(async () => {
      try {
        const response = await fetch(`/api/market/search?q=${encodeURIComponent(value.trim())}`, { signal: controller.signal, cache: "no-store" });
        ...
        if (!controller.signal.aborted) setMessage(error instanceof Error ? error.message : "جست‌وجوی بازار ناموفق بود.");
      } finally {
  ...
  return <div className="space-y-2">
    ...
    {!loading && results.length > 0 && <div className="max-h-56 overflow-y-auto rounded-xl border bg-background p-1 shadow-sm">{results.map((item) => <button key={item.id} type="button" onClick={() => { onSelect(item); setQuery(""); setResults([]); setMessage(""); }} className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-start transition hover:bg-accent"><div className="min-w-0"><div className="type-strong">{item.symbol}</div><div className="truncate type-caption text-muted-foreground">{item.name}</div></div><div className="shrink-0 text-end">{item.priceToman ? <div className="type-label">{formatMoney(item.priceToman, settings.displayUnit, true)}</div> : null}{item.changePercent !== undefined && <div className={item.changePercent >= 0 ? "text-[10px] text-primary" : "text-[10px] text-destructive"}>{item.changePercent >= 0 ? "+" : ""}{new Intl.NumberFormat("fa-IR", { maximumFractionDigits: 2 }).format(item.changePercent)}٪</div>}</div></button>)}</div>}
    {!loading && message && <div className="rounded-xl bg-muted/45 px-3 py-2 type-caption leading-6 text-muted-foreground">{message}</div>}
    <p className="text-[10px] leading-5 text-muted-foreground">جست‌وجوی بورس و صندوق‌های قابل معامله از <MarketSourceLabel source="tindex" /> انجام می‌شود. اگر سرویس در دسترس نباشد، قیمت پشتیبان دستی استفاده می‌شود.</p>
  </div>;
}
