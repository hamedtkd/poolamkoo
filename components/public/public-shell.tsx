  ...
  return <div className="min-h-svh bg-background">
    <header className="sticky top-0 z-30 border-b bg-background/88 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-6xl items-center gap-3 px-4 sm:px-6">
        <Link href="/" className="flex items-center gap-2 rounded-xl px-1.5 py-1 transition hover:bg-accent" aria-label="بازگشت به پولم‌کو"><BrandLogo className="size-9" /><span className="type-label">پولم‌کو</span></Link>
        <nav className="mx-auto hidden items-center gap-1 md:flex">{publicNav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className="flex h-10 items-center gap-2 rounded-xl px-3 text-sm text-muted-foreground transition hover:bg-accent hover:text-foreground"><Icon />{label}</Link>)}</nav>
        <div className="ms-auto flex items-center gap-2"><GithubLink /><Link href="/" className="hidden h-10 items-center gap-2 rounded-xl border px-3 text-sm transition hover:bg-accent sm:flex"><RiArrowRightLine /> برنامه</Link></div>
      </div>
    </header>
