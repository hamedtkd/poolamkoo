...
export async function createRecoverySnapshot(reason: string) {
  ...
  const itemCount = payload.incomes.length + payload.transactions.length + payload.planItems.length + payload.allocations.length + payload.fundMovements.length + payload.loans.length + payload.loanPayments.length;
