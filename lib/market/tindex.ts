...
export class TindexProvider {
  ...
  async getCoreQuotes(): Promise<MarketQuote[]> {
    return parseTindexBoardsPayload(await this.request<TindexBoardsPayload>("/boards", MARKET_CACHE_SECONDS.tindexCoreQuotes));
  }
  async getTargetQuote(target: ExchangeQuoteTarget): Promise<MarketQuote | null> {
    if (target.source === "tindex") {
      const quote = await this.getQuote(target.id);
    ...
    if (!instrument?.priceToman || !(instrument.priceToman > 0)) return null;
    const changePercent = numeric(instrument.changePercent);
    const denominator = 1 + changePercent / 100;
    const changeValueToman = denominator > 0 ? instrument.priceToman - instrument.priceToman / denominator : 0;
    return attachExchangeIdentity({
      symbol: target.symbol,
      name: target.name,
      priceToman: instrument.priceToman,
      changePercent,
      changeValueToman,
  ...
  async getTargetQuotes(targets: readonly ExchangeQuoteTarget[]): Promise<MarketQuote[]> {
    ...
    if (!target) return [];
    const quote = await this.getTargetQuote(target);
    return quote ? [quote] : [];
  }
  async getQuote(marketId: string): Promise<MarketQuote | null> {
    const payload = await this.request<TindexOverviewPayload>(`/stock-market/symbol/${encodeURIComponent(marketId)}/overview`, MARKET_CACHE_SECONDS.tindexExchangeQuote);
  ...
  private async request<T>(path: string, revalidate: number): Promise<T> {
    ...
    try {
      response = await fetch(`${BASE_URL}${path}`, {
        headers: { Authorization: `Bearer ${this.token}`, Accept: "application/json", ...requestDomainHeaders() },
